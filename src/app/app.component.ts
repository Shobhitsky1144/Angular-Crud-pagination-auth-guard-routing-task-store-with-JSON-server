...
export class AppComponent {
  ...
  submit(){
   if(this.taskForms.value.name){
      this.startList.push({id:this.startList.length,name:this.taskForms?.value.name,desc:this.taskForms.value.desc,color:this.taskForms.value.color});
  ...
  completedData(data:any){
    this.progressList=this.progressList.filter((val:any)=>val.id!==data.id)
  ...
  finishData(data:any){
    this.completeList=this.completeList.filter((val:any)=>val.id!==data.id)
  }
